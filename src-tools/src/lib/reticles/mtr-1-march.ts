...
export function drawMtr1March(canvas: ReticleCanvas, input: ReticleRenderInput): void {
  ...
  canvas.line(centerX, centerY - POST_START_MRAD * pxPerMrad, centerX, centerY + POST_START_MRAD * pxPerMrad, linePaint);
  canvas.line(centerX - POST_START_MRAD * pxPerMrad, centerY, centerX + POST_START_MRAD * pxPerMrad, centerY, linePaint);
  ...
  for (const yOffset of [2, 4, 6, 8]) {
    ...
    canvas.line(centerX - halfWidth, centerY + yOffset * pxPerMrad, centerX + halfWidth, centerY + yOffset * pxPerMrad, linePaint);
  }

  const postHalfWidth = 0.25 * pxPerMrad;
  canvas.rect(centerX + POST_START_MRAD * pxPerMrad, centerY - postHalfWidth, centerX + scaledRadius, centerY + postHalfWidth, fillPaint);
  canvas.rect(centerX - scaledRadius, centerY - postHalfWidth, centerX - POST_START_MRAD * pxPerMrad, centerY + postHalfWidth, fillPaint);
  canvas.rect(centerX - postHalfWidth, centerY - scaledRadius, centerX + postHalfWidth, centerY - POST_START_MRAD * pxPerMrad, fillPaint);
  canvas.rect(centerX - postHalfWidth, centerY + (POST_START_MRAD + 2) * pxPerMrad, centerX + postHalfWidth, centerY + scaledRadius, fillPaint);
}
